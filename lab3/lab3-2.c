...
void cycle_memory() {
    ...
    if (mio_en) {
        ...
        if (W) {
           ...
           //  write_memory(datasize_mux(get_DATASIZE(CURRENT_LATCHES.MICROINSTRUCTION),mask_val(CURRENT_LATCHES.IR,14,12),0),URRENT_LATCHES.MAR,CURRENT_LATCHES.MDR) ;
...
void latch_datapath_values() {
    ...
    if (get_LD_PC(CURRENT_LATCHES.MICROINSTRUCTION)) {
        ...
        NEXT_LATCHES.PC = pc_mux(get_PCMUX(CURRENT_LATCHES.MICROINSTRUCTION),CURRENT_LATCHES.PC + 4,CURRENT_LATCHES.PC +BUS- 4);
    }
    /* RESET */
